...
- `data/` : Example input/output files for quick testing
  ...
  - `random_data_taskC/` : Automatically generated preference files for Task C
    - `n1.in`, `n2.in`, ... , `n512.in`
  - `stable_match_results_taskC/` : Stable matching outputs generated by the matcher for Task C
    - `n1.out`, `n2.out`, ... , `n512.out`
...
- `scripts/` : Helper scripts for testing / debugging / scalability experiments
  ...
  - `task_c.py` : Produces timing results and plots.

- `result/` : Runtime measurement results for Task C
  - `taskC_matcher.png` : Matcher runtime vs. problem size.
  - `taskC_verifier.png` : Verifier runtime vs. problem size.
...
To save the matching result to a file (e.g., `example.out`), you can use shell output redirection:

```bash
python3 src/matcher.py data/example.in > data/example.out
```
This command redirects the printed matching output into `data/example.out`, which can then be used as the expected output file for verification.
